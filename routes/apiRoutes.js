...
const writeNewNotes = async (notes) => {
    fs.writeFileSync(
...
module.exports = function (app) {
    ...
    app.get("/api/notes", function (req, res) {
        ...
        app.delete("/api/notes/:id", (req, res) => {
                    ...
                    // let notes = await fs.readFile(path.join(__dirname, "../db/db.json"));
                     fs.readFile(path.join(__dirname, "../db/db.json"), (err, data)=>{
                         const notes = JSON.parse(data);
                         const delId = req.params.id;
                         //created a new const to put the delete function into in order to fix async/sync issues
                         const newNotes = notes.filter(element=>element.id!=delId); //functionality
